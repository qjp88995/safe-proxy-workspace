...
for _ in $(seq 1 20); do
  workspace_user="$(docker compose "${compose_args[@]}" exec -T "${service}" sh -lc 'cat /run/workspace-user 2>/dev/null || true')"
...
workspace_home="$(docker compose "${compose_args[@]}" exec -T "${service}" sh -lc "getent passwd '${workspace_user}' | cut -d: -f6")"
if [ -z "${workspace_home}" ]; then
  echo "workspace home is not ready for user '${workspace_user}' in service '${service}'" >&2
  exit 1
fi

workspace_dir="$(docker compose "${compose_args[@]}" exec -T "${service}" sh -lc 'cat /run/workspace-dir 2>/dev/null || true')"
if [ -z "${workspace_dir}" ]; then
  workspace_dir="${workspace_home}/workspace"
fi

exec docker compose "${compose_args[@]}" exec \
  -u "${workspace_user}" \
  -w "${workspace_dir}" \
